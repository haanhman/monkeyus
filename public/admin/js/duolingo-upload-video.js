...
function upload_video(uploaded) {
    ...
    if(video_replace == 0) {
        xhr.open('POST', $('#base_url').val() + '/backend/ajax/duolingvideo', true);
    } else {
        xhr.open('POST', $('#base_url').val() + '/backend/ajax/duolingvideo?replace=1', true);
    }
    ...
    xhr.onload = function() {
        if (this.status == 200) {            
            ...
            if (json.status == 1) {
                ...
                if (json.type == 'image') {
                    $('li#file_' + json.index).find('.thumb img').show();
                    $('li#file_' + json.index).find('div.title').html('<label><input img_id="' + json.video_id + '" class="image_type" checked="" type="checkbox" value="1"> ảnh chụp</label>');
                }
                add_card(json.index);
            } else {
                leng--;
